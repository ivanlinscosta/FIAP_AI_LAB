# FIAP AI Lab — Guia Completo

Documentação oficial do laboratório de IA da disciplina **Tools, Automations and Workflows**.

Este guia explica, do zero, como o laboratório funciona, como o **professor** usa e como o **aluno** usa pela primeira vez.

---

## Sumário

1. [O que é o FIAP AI Lab](#1-o-que-é-o-fiap-ai-lab)
2. [Por que um gateway de IA? (conceito)](#2-por-que-um-gateway-de-ia)
3. [Arquitetura do sistema](#3-arquitetura-do-sistema)
4. [URLs de acesso](#4-urls-de-acesso)
5. [Modelos de IA disponíveis](#5-modelos-de-ia-disponíveis)
6. [PARTE A — Guia do Aluno (primeira vez)](#parte-a--guia-do-aluno-primeira-vez)
7. [PARTE B — Guia do Professor](#parte-b--guia-do-professor)
...
1. O professor disponibiliza um **Custom Connector** (arquivo de template que fica em `power-automate/`);
...
- Evite loops que chamam a API repetidamente sem necessidade (muito comum no n8n);
...
2. Faça login com as credenciais de administrador (usuário `professor` e a senha definida no Railway);
...
- O arquivo `.env` (contém chaves sensíveis);
- `OPENAI_API_KEY` (chave real da OpenAI);
- `LITELLM_MASTER_KEY` (chave mestre do gateway);
...
2. Teste uma chamada com a chave de um grupo (ex.: `python3 scripts/smoke_test.py` com `FIAP_GROUP_KEY` definida);
...
2. Fique atento a alertas de anomalia (loops acidentais de alunos);
3. Tenha um grupo "reserva" (extra) para emergências;

### Depois da aula

1. Exporte o CSV com o consumo da aula;
2. Bloqueie os grupos que não usarão mais;
3. Anote o que precisará ser ajustado (budget, nº de grupos) para a próxima aula.
...
2. Aumente o budget (ex.: de US$ 5 para US$ 10);
...
3. Salve a nova chave (aparece uma única vez);
...
5. O dashboard deve ter **senha forte** (variável `ADMIN_PASSWORD`);
6. Revogue (regenerando chave) qualquer chave que vaze.
...
---
*Documentação oficial do FIAP AI Lab — uso educacional.*